...
public interface PetsApi {
     ...
     *
     * By default, this implementation returns an empty {@link Optional}. Subclasses
     * may override this method to provide a non-empty {@link Optional} containing
     * the actual {@link NativeWebRequest} if available.
     *
     ...
     * @return Null response. (status code 201) or unexpected error, (status code
     *         200)
     */
    @Operation(operationId = "createPets", summary = "Create a pet.", description = "Create a pet in the store with the given details.", tags = {
            "pets" }, responses = { @ApiResponse(responseCode = "201", description = "Null response."),
     ...
     * @return A paged array of pets. (status code 200) or unexpected error. (status
     *         code 200)
     */
    @Operation(operationId = "listPets", summary = "List all pets.", description = "Get a paged list of pets.", tags = {
            "pets" }, responses = {
    ...
    default ResponseEntity<List<Pet>> listPets(
            @Parameter(name = "limit", description = "How many items to return at one time (max 100).", in = ParameterIn.QUERY) @Valid @RequestParam(required = false) Integer limit) {
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
     ...
     */
    @Operation(operationId = "showPetById", summary = "Info for a specific pet.", description = "Get a specific pet by ID.", tags = {
            "pets" }, responses = {
    ...
    default ResponseEntity<Pet> showPetById(
            @Parameter(name = "petId", description = "The id of the pet to retrieve.", required = true, in = ParameterIn.PATH) @PathVariable String petId) {
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
