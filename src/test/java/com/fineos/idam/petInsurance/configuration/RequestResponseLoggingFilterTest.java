 ...
 *
 * <p>
 * This class verifies that HTTP request and response logging works as expected,
 * ensuring that headers are set, logging conditions are respected, and response
 * bodies are processed correctly.
 */
...
class RequestResponseLoggingFilterTest {
    ...
    /**
     * Sets up the test environment before each test case. Initializes the filter,
    ...
    /**
     * Tests that correlation headers are correctly set in the response when they
    ...
    void testHeadersAreSetWhenMissing() throws ServletException, IOException {
        ...
        Assertions.assertEquals(RequestResponseLoggingFilter.UNKNOWN,
        ...
        Mockito.verify(filterChain).doFilter(Mockito.any(HttpServletRequest.class),
                Mockito.any(HttpServletResponse.class));
    }

    /**
     * Tests that correlation headers from the request are passed through to the
     * response.
     */
    @Test
    ...
    /**
     * Tests that request and response logging is disabled when the
     * `logging.http-payload.enabled` property is set to false.
     */
    @Test
    ...
    void testLoggingCapturesRequestAndResponseBody() throws ServletException, IOException {
        ...
        Mockito.verify(mockChain).doFilter(Mockito.any(ContentCachingRequestWrapper.class),
