 ...
 * Includes methods for verifying getter/setter correctness, validating hashCode
 * and equals implementations, and generating JSON representations of objects.
 *
...
public final class AssertionUtilDto<T> {
    ...
    /**
     * An {@link ObjectMapper} instance for serializing and deserializing objects to
     * and from JSON. Configured with the JavaTime module and a specific date
     * format.
     */
     ...
     *
     * @throws UnsupportedOperationException Always throws an exception since this
     *                                       is a utility class.
     */
     ...
     * If the objects are instances of {@link OffsetDateTime}, the comparison is
     * done by comparing their {@link java.time.Instant} values.
     *
    ...
    public static <T> void verifySetterGetters(T actual, T expected) {
        try {
            if (actual instanceof OffsetDateTime time && expected instanceof OffsetDateTime time1) {
                Assertions.assertEquals(time1.toInstant(), time.toInstant(),
    ...
    /**
     * Verifies the correctness of equals() and hashCode() implementations for the
     * given objects. Performs checks for equality and inequality between objects of
     * the same type, non-null constraints, and field-by-field equality using
     * reflection.
     *
        ...
        for (int index = 0; index < fieldCount; index++) {
            try {
                ...
                if (index < fieldCount - 1) {
                    Assertions.assertNotEquals(o2, o3, "o2 and o3 objects should not have been equal. "
                            + "This occurred while comparing field " + o2Fields[index].getName());
                } else {
                    Assertions.assertEquals(o2, o3, "o2 and o3 objects should have been equal.");
                }
            } catch (IllegalArgumentException | IllegalAccessException e) {
                Assertions.fail("Error encountered while setting the field '" + o2Fields[index].getName()
                        + "' using reflection", e);
            }
        }
    }
    ...
    /**
     * Generates the JSON representation of the given object using
     * {@link ObjectMapper}.
     *
     ...
     *         null.
     * @throws RuntimeException If there is an error while accessing the object's
     *                          fields.
     */
