...
public final class AssertionUtil<T> {
    ...
    /**
     * A singleton instance of {@link ObjectMapper} used for JSON serialization and deserialization tasks.
     */
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    /**
     * Private constructor to prevent instantiation of this utility class.
     * Asserting utility methods are meant to be accessed statically.
     * Invoking this constructor will throw an {@link UnsupportedOperationException}.
     */
    ...
    /**
     * Asserts that two lists of objects are equal by comparing their JSON representations.
     *
     ...
     * @param expected         a parameter object encapsulating remaining expected values for clarity
     */
    public static void assertPageEquals(
            Page<?> result, long totalElements, int pageNumber, int pageSize,
        ...
        Assertions.assertEquals(totalElements <= (long) pageSize * (pageNumber + 1), result.isLast(), "isLast status does not match");
        ...
        if (expected.hasNext()) {
            ...
            Assertions.assertEquals(expected.nextOrLastPageable(), result.getPageable(), "Next or last pageable mismatch");
        }

        Pageable actualPreviousPageable = result.previousPageable();
        if (expected.hasPrevious()) {
            Assertions.assertEquals(expected.previousPageable(), actualPreviousPageable, "Previous pageable does not match");
        } else {
            Assertions.assertEquals(expected.previousOrFirstPageable(), result.getPageable(), "Previous or first pageable mismatch");
        }
    }
     ...
     * @param nextOrLastPageable      the {@link Pageable} object for the next or last page
     * @param nextPageable            the {@link Pageable} object for the next page
     * @param previousOrFirstPageable the {@link Pageable} object for the previous or first page
     * @param previousPageable        the {@link Pageable} object for the previous page
     * @param sort                    the {@link Sort} object indicating the sort order
     */
    public record PageAssertOptions(
            boolean hasNext, boolean hasPrevious, Pageable nextOrLastPageable,
            Pageable nextPageable, Pageable previousOrFirstPageable,
            Pageable previousPageable, Sort sort) {
            //Empty block
    }
}
