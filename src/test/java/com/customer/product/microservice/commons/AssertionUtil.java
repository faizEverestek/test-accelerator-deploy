package com.customer.product.microservice.commons;
...
public final class AssertionUtil<T> {
    ...
    /**
     * Private constructor to prevent instantiation of this utility class. Asserting utility methods are meant to be
     * accessed statically. Invoking this constructor will throw an {@link UnsupportedOperationException}.
     */
    ...
    public static void assertPageEquals(
        ...
        Assertions.assertEquals(
                totalElements <= (long) pageSize * (pageNumber + 1), result.isLast(), "isLast status does not match");
        ...
        if (expected.hasNext()) {
            ...
            Assertions.assertEquals(
                    expected.nextOrLastPageable(), result.getPageable(), "Next or last pageable mismatch");
        }

        Pageable actualPreviousPageable = result.previousPageable();
        if (expected.hasPrevious()) {
            Assertions.assertEquals(
                    expected.previousPageable(), actualPreviousPageable, "Previous pageable does not match");
        } else {
            Assertions.assertEquals(
                    expected.previousOrFirstPageable(), result.getPageable(), "Previous or first pageable mismatch");
        }
    }
